...
class DlController extends Controller
{
	...
	public function actionBlocks($position_id) 
	{
		...
		$post_guid = urlencode($_GET['guid']);
		$blocks = Block::model()->findAllByAttributes(array('block_position_id'=>$position_id, 'status_id'=>1));
		foreach ($blocks as $block) {
			if ($this->showBlock($block['id'],$post_guid)) {
				$this->renderBlock($block);
			}
		}
	}

	private function showBlock($block_id,$post_guid)
	{
		$post = Post::model()->findByAttributes(array('guid'=>$post_guid));
		...
		switch ($post->post_type_id)
		{
			...
			case 2: // ako je stranica
				...
				if (in_array($post['id'],$block_pages)) {
					return true;
					break;
				} else {
	...
	public function loadModel($guid)
	{
		
		$model=Post::model()->findByAttributes(array('guid'=>urlencode($guid)));
		if ($model===null) {
